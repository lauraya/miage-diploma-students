...
public abstract class AbstractDiplomaGenerator implements DiplomaGenerator {
	 ...
	 */
	@Override
	public InputStream getContent() throws GenerateException{
		...
		try (ByteArrayOutputStream bos = new ByteArrayOutputStream();) {
			...
			throw new GenerateException("failed to generate the file to stream to", e);
		}

	}

	protected void writeToStream(OutputStream os) throws GenerateException{
		...
		try {
			...
			throw new GenerateException("failed to generate Document", e);
		} finally {
			document.close();
		}
	}
